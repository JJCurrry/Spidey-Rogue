...
- 下一步候选（M27 序列帧 Sprite 已落地）：① 交互式射灯/碎灯/墙边开关的目标选择 UI（M14/M16/M19 的玩家可控版）；② 更多蜘蛛侠招式（AoE 蛛网束缚 / 可指定落点摆荡位移）；③ ~~主题美术换 `tiles/*.png` 序列帧 Sprite（已完成，见 M27）~~；④ 网页化（Pygbag 把 Pygame 打包成 wasm，零改核心）。
