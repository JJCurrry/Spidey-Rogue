...
- M26 存档 / 读档（已完成，见 `docs/工单/T-026-存档读档.md` / `docs/adr/ADR-022-存档读档.md`）：把「关掉即丢」变成「可随时存、关掉再读档接着玩」——`Game` 新增 `to_dict`/`apply_state`/`from_dict`/`save`/`load`/`load_into`，`RandomSource` 新增 `get_state`/`set_state`（完整保存 PRNG 内部状态）；存「seed + PRNG 内部状态 + 全部玩法状态」，读档后续演化与「从未存读、并行推进」完全一致（不变量 #26）；`main.py` 的 `_handle_key` 增大写 `S`/`L` 交互存读档（默认路径 `savegame.json`，已加 `.gitignore`）；opt-in 默认不触发，不加参数与 M25 逐字节一致；gate 564→574 全绿（不变量 #26）
- 下一步候选（M26 存档/读档已落地）：① 交互式射灯/碎灯/墙边开关的目标选择 UI（M14/M16/M19 的玩家可控版）；② 更多蜘蛛侠招式（AoE 蛛网束缚 / 可指定落点摆荡位移）；③ 主题美术换 `tiles/*.png` 序列帧 Sprite（预渲染已按帧组织，接口已留好）；④ 网页化（Pygbag 把 Pygame 打包成 wasm，零改核心）。
