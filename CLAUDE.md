...
- 下一步候选（M17 已完成「环境/完整光照场分离」，剩余两条待立项）：让光照也影响蜘蛛感应半径（SPIDER_SENSE_RADIUS=4 穿墙预警目前不受光照约束；主题上是否该受光照约束需先定调）；灯开关独立实体（墙上开关 tile，需新实体类型 + 新渲染字形）；另 M17 留「怪物感知改用环境场」作为 opt-in 后续项（修正被动微光把暗处怪照成近视眼的隐性副作用，需重跑 --light/--flashlight 演示）
