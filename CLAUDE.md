...
- 下一步候选（M25 Boss 战与胜利闭环已收口「一局胜负」）：① 交互式射灯/碎灯/墙边开关的目标选择 UI（M14/M16/M19 的玩家可控版）；② 存档/读档（save/load）；③ 更多蜘蛛侠招式（AoE 蛛网束缚 / 可指定落点摆荡位移）；④ 主题美术换 `tiles/*.png` 序列帧 Sprite（预渲染已按帧组织，接口已留好）；⑤ 网页化（Pygbag 把 Pygame 打包成 wasm，零改核心）。
