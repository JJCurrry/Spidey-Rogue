"""M16 可破坏的灯（蛛网射碎房间中心灯泡，一次性不可逆）。

核心红线覆盖：
- 不变量 #1 随机仅经 RandomSource（destroy_light 纯状态操作，不引入随机模块、不消耗 rng）
- 不变量 #2 回合确定性（破坏只改光照场 ⇒ 同状态同场；destroy 不改写玩法状态）
- 不变量 #8 渲染纯净性（破坏只改光照场与可见集合，不改 render 字形）
- 不变量 #9 延伸：破坏只移除光源（不新增）⇒ 光照场只变暗 ⇒ 有效半径恒 ≤ SIGHT_RADIUS(8)
  ⇒「怪看得见你 ⇒ 你看得见它」硬性质不破
- 不变量 #10 延伸：碎裂轻响仍只经 emit_noise → Monster.alert(cause=CAUSE_SOUND) 唯一入口生效；
  声源在**灯处**（不在玩家处）⇒ 调虎离山成立
- 不变量 #12/#14 延伸：破坏移除房间灯 ⇒ 房间变暗 ⇒ 怪物感知只缩短不放大（恒 ≤ MONSTER_SIGHT_RADIUS）
- 不变量 #16：可破坏的灯纯几何、零随机、默认关闭
...
from rogue.game import (Game, WEB_LIGHT_RANGE, NOISE_SHATTER_BULB,
                        CAUSE_SOUND)
from rogue.level import Level, Room
from rogue.rng import RandomSource
from rogue.tiles import FLOOR, WALL

# 一间房 + 走廊：玩家在房间内，灯在房间中心
# 房间 (1,1)~(10,3)，中心 (6,2)；玩家在 (1,1) 房间内
_ROOM_INNER = [
    "###########",
    "#@........#",
    "#.........#",
...
class TestDestroyDefault(unittest.TestCase):
    """M16 默认关闭：light=False ⇒ destroy_light 恒返回 False、no-op（不变量 #16）。"""

    def test_light_off_destroy_returns_false(self):
        g = _make(_ROOM_INNER, rooms=[_ROOM_INNER_ROOM], light=False)
        cx, cy = _ROOM_INNER_ROOM.center
        self.assertFalse(g.destroy_light(cx, cy))
    ...
    def test_light_off_can_destroy_returns_false(self):
        g = _make(_ROOM_INNER, rooms=[_ROOM_INNER_ROOM], light=False)
        cx, cy = _ROOM_INNER_ROOM.center
        self.assertFalse(g.can_destroy_light(cx, cy))

    def test_procedural_default_has_no_destroyed_lights(self):
        g = Game.procedural(RandomSource(seed=19), depth=1)
        self.assertEqual(g.destroyed_lights, set())
        self.assertFalse(g.light_enabled)


class TestDestroyGeometry(unittest.TestCase):
    """can_destroy_light 的五条硬约束（纯几何、零随机，不变量 #16）。"""

    def test_valid_room_center_in_range(self):
        # 玩家在 (1,1)，房间中心 (6,2)，切比雪夫距离 5 ≤ WEB_LIGHT_RANGE(6) ⇒ 可碎
    ...
    def test_non_room_center_target(self):
        ...
        self.assertFalse(g.can_destroy_light(3, 2))
    ...
    def test_out_of_range(self):
        ...
        self.assertFalse(g.can_destroy_light(8, 3))

    def test_at_range_boundary(self):
        # 切比雪夫距离恰好 = WEB_LIGHT_RANGE(6) ⇒ 可碎（闭区间）
        rows = [
            "#################",
            "#@..............#",
            "#################",
        ]
        room = Room(0, 1, 15, 1)  # center = (7,1)，距 (1,1) = 6
        g = _make(rows, rooms=[room], light=True)
        self.assertEqual(room.center, (7, 1))
        self.assertTrue(g.can_destroy_light(7, 1))
    ...
    def test_wall_blocks_line_of_sight(self):
        ...
        room = Room(4, 1, 6, 1)  # center = (7,1)
        g = _make(rows, rooms=[room], light=True)
        self.assertFalse(g.can_destroy_light(7, 1))

    def test_standing_under_light_can_destroy(self):
        # 玩家站在灯正下方（距离 0）⇒ 伸手就能够到灯泡
    ...
    def test_already_destroyed_not_destroyable(self):
        # 已碎的灯泡不能再碎（不可重复消耗回合）
        g = _make(_ROOM_INNER, rooms=[_ROOM_INNER_ROOM], light=True)
        cx, cy = _ROOM_INNER_ROOM.center
        self.assertTrue(g.destroy_light(cx, cy))
        self.assertFalse(g.can_destroy_light(cx, cy))


class TestDestroyField(unittest.TestCase):
    """破坏后光照场变化（M11 联动：房间彻底变暗；M13 联动：玩家视野缩短）。"""

    def test_destroy_darkens_room(self):
        # 房间亮着时中心 LIT；破坏后变暗（DARK 或 DIM，取决于微光是否够到）
        g = _make(_ROOM_INNER, rooms=[_ROOM_INNER_ROOM], light=True, fov=True)
        cx, cy = _ROOM_INNER_ROOM.center
        ...
        # 灯碎后房间灯没了，只剩玩家微光：中心 (6,2) 距玩家 (1,1) 切比雪夫 5 > PLAYER_GLOW_RADIUS(4) ⇒ DARK
        self.assertLess(after, before)

    def test_destroy_removes_source_from_list(self):
        g = _make(_ROOM_INNER, rooms=[_ROOM_INNER_ROOM], light=True)
        cx, cy = _ROOM_INNER_ROOM.center
    ...
    def test_destroy_irreversible(self):
        # 破坏是一次性不可逆：再调用 destroy_light 返回 False（灯已碎，无可再碎）
        g = _make(_ROOM_INNER, rooms=[_ROOM_INNER_ROOM], light=True)
        cx, cy = _ROOM_INNER_ROOM.center
        self.assertTrue(g.destroy_light(cx, cy))
        lut_after_first = dict(g.light_field)
        # 第二次破坏：can_destroy_light 应 False ⇒ destroy_light 返回 False、场不变
        self.assertFalse(g.destroy_light(cx, cy))
        self.assertEqual(g.light_field, lut_after_first)
    ...
    def test_other_rooms_unaffected(self):
        ...
        g.destroy_light(*ca)  # 碎 A
        b_after = g.light_level_at(*cb)
        self.assertEqual(b_before, b_after)

    def test_destroy_shortens_monster_sight(self):
        # 破坏后房间变暗 ⇒ 怪物感知半径缩短（M11 联动）
        ...
        g.destroy_light(cx, cy)  # 碎灯
        # 灯灭后：怪物所在格变暗 ⇒ 感知半径缩短
        self.assertLess(monster_sight_radius(g.light_level_at(cx, cy)),
                        MONSTER_SIGHT_RADIUS)

    def test_destroy_shortens_player_sight_into_room(self):
        # 破坏后房间格子变暗（M13 联动：暗处目标格视野半径缩短）
        g = Game.procedural(RandomSource(seed=19), depth=1, light=True, fov=True)
        for room in g.rooms:
            cx, cy = room.center
            if not g.can_destroy_light(cx, cy):
                continue
    ...
    def test_destroy_after_toggle_stays_dark(self):
        # 先关灯（switched）再碎灯 ⇒ 永久黑暗、不可恢复；destroy 不会把它「变回亮」
        g = _make(_ROOM_INNER, rooms=[_ROOM_INNER_ROOM], light=True)
        cx, cy = _ROOM_INNER_ROOM.center
        g.toggle_light(cx, cy)  # 关
        self.assertFalse(g.light_is_on(cx, cy))
        self.assertTrue(g.destroy_light(cx, cy))  # 碎
        self.assertFalse(g.light_is_on(cx, cy))
        self.assertIn((cx, cy), g.destroyed_lights)
        # 再 toggle（应为 no-op）：cannot 因为它已碎
        self.assertFalse(g.can_toggle_light(cx, cy))


class TestDestroyDeterminism(unittest.TestCase):
    """不变量 #2：同 seed + 同输入 ⇒ 同光照场；破坏不消耗 RandomSource（#16）。"""

    def test_destroy_does_not_consume_random(self):
        # 破坏是纯状态操作 ⇒ 不消耗 rng
        ...
        a.destroy_light(cx, cy)  # 碎灯
        self.assertEqual([a.rng.int(0, 999) for _ in range(5)],
                         [b.rng.int(0, 999) for _ in range(5)])

    def test_same_seed_same_field_after_destroy(self):
        def field_after_destroy(seed):
            g = Game.procedural(RandomSource(seed=seed), depth=1, light=True, fov=True)
            cx, cy = g.rooms[0].center
            g.destroy_light(cx, cy)
            return dict(g.light_field)

        self.assertEqual(field_after_destroy(19), field_after_destroy(19))

    def test_destroy_idempotent_field(self):
        # 碎灯后光照场稳定：再算几次结果一样
...
class TestDestroyNoise(unittest.TestCase):
    """M8 联动：破坏发出 NOISE_SHATTER_BULB 响动，声源在灯处（不在玩家处）。"""
    ...
    def test_noise_emitted_when_noise_on(self):
        ...
        self.assertEqual(before_loudness, 0)  # 破坏前没有发出过动静
    ...
    def test_failed_destroy_no_noise(self):
        # 够不着（非房间中心）⇒ destroy 返回 False、不发声
        g = _make(_ROOM_INNER, rooms=[_ROOM_INNER_ROOM], light=True, noise=True)
        before = g.last_noise_loudness
        g.destroy_light(3, 2)  # 不是房间中心
        self.assertEqual(g.last_noise_loudness, before)


class TestDestroyRenderPurity(unittest.TestCase):
    """不变量 #8：破坏只改光照场与可见集合，不改 render 字形。"""

    def test_fov_off_render_identical(self):
        # fov=False ⇒ 全图渲染，破坏不改字形（只改 colorize 上色层的明暗）
        ...
        cx, cy = _ROOM_INNER_ROOM.center
        a.destroy_light(cx, cy)  # a 碎灯、b 不碎
    ...
    def test_render_does_not_touch_world_state(self):
        # destroy_light 后 render() 不改写任何状态（#8：world state 前后快照比对）
...
class TestDestroyReset(unittest.TestCase):
    """换层重置：descend → load_level 清空 destroyed_lights（新楼层 = 新灯泡）。"""

    def test_load_level_clears_destroyed_lights(self):
        g = Game.procedural(RandomSource(seed=12), depth=1, light=True)
        cx, cy = g.rooms[0].center
        g.destroy_light(cx, cy)
        self.assertEqual(len(g.destroyed_lights), 1)
        g.grid[g.py][g.px] = FLOOR
        g.px, g.py = g.stairs
        g.grid[g.py][g.px] = "@"
        g.update_fov()
        self.assertTrue(g.descend())
        self.assertEqual(g.destroyed_lights, set())  # 新楼层清空
...
class TestDestroySymmetry(unittest.TestCase):
    """不变量 #9 硬性质在 M16 下仍成立：怪看得见你 ⇒ 你看得见它。"""

    def test_monster_sees_you_you_can_see_after_destroy(self):
        g = Game.procedural(RandomSource(seed=11), depth=1,
                            fov=True, stealth=True, light=True)
        if g.rooms:
            cx, cy = g.rooms[0].center
            if g.can_destroy_light(cx, cy):
                g.destroy_light(cx, cy)
        for _ in range(40):
            g.monster_turn()
            g.update_fov()
            for m in g.monsters:
                if m.alive and g.monster_can_see_player(m):
                    self.assertTrue(g.is_visible(m.x, m.y),
                                    f"{m.name}@{m.x},{m.y} 看得见你、你却看不见它（碎灯后）")

    def test_symmetry_across_seeds_with_destroy(self):
        for seed in (3, 7, 11, 19, 23):
            ...
            for _ in range(30):
                ...
                for m in g.monsters:
                    if m.alive and g.monster_can_see_player(m):
                        self.assertTrue(g.is_visible(m.x, m.y),
                                        f"seed={seed} {m.name}@{m.x},{m.y} 幽灵猎手（碎灯后）")

    def test_destroy_only_shortens_never_amplifies(self):
        # 碎灯只移除光源 ⇒ 光照场只变暗（等级只降不升）⇒ 有效半径只缩不放
        g = Game.procedural(RandomSource(seed=19), depth=1, light=True, fov=True)
        for room in g.rooms:
            cx, cy = room.center
            if g.can_destroy_light(cx, cy):
...
class TestDestroyInteractionWithToggle(unittest.TestCase):
    """破坏与 M14 翻转开关的交互：碎了的灯不能再 toggle；关着的灯可碎成永久黑暗。"""

    def test_destroyed_light_not_toggleable(self):
        g = _make(_ROOM_INNER, rooms=[_ROOM_INNER_ROOM], light=True)
        cx, cy = _ROOM_INNER_ROOM.center
        g.destroy_light(cx, cy)
        self.assertFalse(g.can_toggle_light(cx, cy))  # 碎了的灯无法再 toggle

    def test_toggle_off_then_destroy_permanent(self):
        # 先 toggle 关灯（switched），再 destroy ⇒ switched 被清、destroyed 生效，永久黑暗
        g = _make(_ROOM_INNER, rooms=[_ROOM_INNER_ROOM], light=True)
        cx, cy = _ROOM_INNER_ROOM.center
        g.toggle_light(cx, cy)  # 关
        self.assertIn((cx, cy), g.switched_lights)
        g.destroy_light(cx, cy)  # 碎
        self.assertNotIn((cx, cy), g.switched_lights)
        self.assertIn((cx, cy), g.destroyed_lights)
        # 再次 destroy 是 no-op
        self.assertFalse(g.destroy_light(cx, cy))

    def test_destroy_then_toggle_is_noop(self):
        # 先 destroy 再尝试 toggle ⇒ can_toggle_light False，toggle 返回 False、场不变
        g = _make(_ROOM_INNER, rooms=[_ROOM_INNER_ROOM], light=True)
        cx, cy = _ROOM_INNER_ROOM.center
        g.destroy_light(cx, cy)
...
class TestDestroyQuery(unittest.TestCase):
    ...
    def test_light_off_after_destroy(self):
        g = _make(_ROOM_INNER, rooms=[_ROOM_INNER_ROOM], light=True)
        cx, cy = _ROOM_INNER_ROOM.center
        g.destroy_light(cx, cy)
        self.assertFalse(g.light_is_on(cx, cy))

    def test_light_on_after_failed_destroy(self):
        # 失败破坏（非房间中心）不应改变灯状态
        g = _make(_ROOM_INNER, rooms=[_ROOM_INNER_ROOM], light=True)
        cx, cy = _ROOM_INNER_ROOM.center
        g.destroy_light(3, 2)  # 不是房间中心
        self.assertTrue(g.light_is_on(cx, cy))  # 中心灯仍亮
...
class TestDestroyMultipleRooms(unittest.TestCase):
    """多房间场景：逐个碎灯、各不影响。"""

    def test_destroy_two_rooms_independently(self):
        g = Game.procedural(RandomSource(seed=19), depth=1, light=True)
        destroyable = [r.center for r in g.rooms if g.can_destroy_light(*r.center)]
        if len(destroyable) < 2:
            self.skipTest("本 seed 没有两个可碎灯的房间")
        ca, cb = destroyable[0], destroyable[1]
        g.destroy_light(*ca)
        self.assertIn(ca, g.destroyed_lights)
        self.assertNotIn(cb, g.destroyed_lights)
        self.assertTrue(g.light_is_on(*cb))  # B 的灯仍亮
        g.destroy_light(*cb)
        self.assertIn(cb, g.destroyed_lights)
        self.assertEqual(len(g.destroyed_lights), 2)

    def test_all_rooms_destroyable_in_procedural(self):
        g = Game.procedural(RandomSource(seed=19), depth=1, light=True)
        destroyable = [r for r in g.rooms if g.can_destroy_light(*r.center)]
        self.assertGreater(len(destroyable), 0,
                           "程序化楼层里至少该有一个房间的灯能碎")
