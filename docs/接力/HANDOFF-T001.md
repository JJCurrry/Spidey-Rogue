...
## 当前状态
- 整体进度：M1 已完成（治理脚手架 + 格子移动）；M2 已完成（战斗系统）；M3 已完成（怪物 AI）；M4 已完成（道具与背包）；M5 已完成（程序化关卡）；M6 已完成（视野 / 渲染层）；M7 已完成（怪物视野与潜行）；M8 已完成（噪音与听觉）；**M9 已完成（主动制造响动 / 皇后区垃圾桶盖）**；**M10 已完成（ANSI 颜色高亮）**；**M11 已完成（光照衰减 / 明暗梯度）**；**M12 已完成（随身手电 / 动态光源）**；**M13 已完成（光照影响玩家自身视野）**；**M14 已完成（可开关房间灯 / 蛛网射灯拉链）**。
...
- [x] M14 可开关房间灯：`Game` 新增 `switched_lights: set[Tile]`（已关灯的房间中心坐标）+ `can_toggle_light(x,y)` / `toggle_light(x,y)` / `light_is_on(x,y)`；蛛网射中房间中心灯的拉链翻转该房间的灯（关灯变暗 / 开灯恢复）；`_light_sources()` 跳过 `switched_lights` 里的房间 ⇒ 关灯只移除光源（不新增）⇒ 光照场只变暗或恢复、有效半径恒 ≤ `SIGHT_RADIUS`(8) ⇒ #9/#12/#14 不破；四条硬约束：光照开启 + 目标是房间中心 + 切比雪夫 ≤ `WEB_LIGHT_RANGE=6`（含脚下）+ 玩家看得见灯（`has_line_of_sight`）；M8 联动：拉链轻响 `NOISE_TOGGLE_LIGHT=3` 从**灯处**传出（不在玩家处）⇒ 调虎离山成立（比垃圾盖 9 轻、只惊动近处的人）；`toggle_light` 只翻集合 + 重算场（`update_fov`），不改写 `grid`/HP/背包/怪物状态（与 `toggle_flashlight` 同一零副作用哲学）；换层（`load_level`）重置 `switched_lights`；`render()` 字形一字不差（关灯只改光照场与可见集合，不改字形，#8 延伸）；复用 `light` 开关不新增开关、只在 `light=True` 时有意义（`light=False` ⇒ `toggle_light` 恒返回 False、no-op）；`main.py` 在 `--light --stealth` 下主动射灭未察觉敌人所在房间的灯（演示 seed 19 下共关 5 盏灯、三层均通关）；`tests/test_light_switch.py`（42 例，1 skip）；ADR-010；不变量 #15（可开关房间灯：纯几何零随机、默认关闭、只移除光源不新增、声源在灯处、toggle 零副作用、换层重置）；测试数 342 → 384，门禁四道门 + 评审流水线全绿（无 HIGH），七条演示均通关，不加 `--light` 的前三条与 M13 逐字节一致 — commit `<待回填>`

## 下一步指令（给下一个会话 / M15）
0. 已完成的 M14 关键结论（**先读，能省半天**）：
   - **关灯只移除光源、不新增**：`_light_sources()` 跳过 `switched_lights` 里的房间中心，
     关灯把房间灯从光源列表里拿掉 ⇒ 房间变暗（M11 缩短怪物感知 + M13 缩短玩家视野——双刃）。
     开灯把光源加回去 ⇒ 恢复明亮。光照场只变暗或恢复，不会比 M11 更亮 ⇒ #9/#12/#14 不破。
   - **翻转式（toggle）而非一次性破坏**：「可开关」= 可重复翻转。蛛网射中灯的拉链——
     拉一下关、再拉一下开。状态用 `switched_lights: set[Tile]`（已关灯的房间中心坐标）。
   - **声源在灯处、不在玩家处**：拉链轻响 `NOISE_TOGGLE_LIGHT=3` 从灯的位置传出（不是玩家脚下）
     ⇒ 可作调虎离山的轻量手段（比垃圾盖 9 轻、只惊动近处的人，但同时把房间弄暗）。
     与 M8「被蛛网弹缠住的怪挣扎出声」同一原则（声源未必是玩家）。
   - **不改 M6「进房间点亮整间」规则**：关灯后玩家站在该房间里仍能点亮整间（M6 规则保留）；
     M14 只缩短「跨房间」视野（从走廊看进暗房间看不远）。改这条规则需传 `switched_lights` 给
     `visible_tiles`、动 M6/M13 既有规格，风险高、收益有限。
   - **M14 只在 `light=True` 时有意义**：`light=False` ⇒ `_light_sources` 不含房间灯（关不关都一样）
     ⇒ `toggle_light` 恒返回 False（no-op）。不新增开关，复用 `light`（与 M13 同一哲学）。
   - **演示 AI 只在 `--light --stealth` 下主动关灯**：stealth 关闭时怪物恒为已察觉，
     关灯不缩短 AI 行为（只缩短 `monster_can_see_player` 的几何查询，但 `update_awareness` 不跑）
     ⇒ 不加 `--stealth` 的 `--light` / `--flashlight` 演示与 M13 逐字节一致（仅图例/说明文案差异）。
1. 读 `CLAUDE.md` → 拉 `docs/工单/T-015*` 或新建。下一步候选：
   - **M3 老追击路径修正**（已登记为后续项：`_step_toward` 只比曼哈顿、平局原地打转；
     M7 潜行分支已用切比雪夫二次关键字修正，但改老路径会改写 M6 既有演示行为 ⇒ 需单独基线化 + 重配平。
     HANDOFF/MEMORY 已实测量化：只修会从 26/30 掉到 22/30，必须配平怪物攻击 1~3→1~2 ⇒ 28/30）；
