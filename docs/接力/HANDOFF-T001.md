...
1. 读 `CLAUDE.md` → （T-019 / T-020 已建，见 `docs/工单/T-019-灯开关独立实体.md`、`docs/工单/T-020-光照影响蜘蛛感应半径.md` 与对应 ADR-015 / ADR-016）。下一步候选（M19 完成后剩余唯一一条「光照影响蜘蛛感应半径」已落地为 M20）：
   ...
   - **（M21 已完成：可键盘操作玩法；M22 已完成：Pygame GUI 渲染层；M23 已完成：Spider-Man 主题化 GUI 渲染）**：M6–M23 的「视野 / 潜行 / 听觉 / 光照 / 蜘蛛感应 + 可键盘操作 + 窗口渲染 + 蜘蛛侠主题美术」体系已闭环，游戏从「只能看 ASCII 终端 demo」变成「能在真实窗口里手动玩、且看起来像蜘蛛侠」。后续新里程碑仍需先登记工单 + ADR + 不变量后再开工。候选方向（按优先级）：
     ...
     - **主题美术再升级（M23 续）**：当前美术是几何程序化（零素材依赖、确定性满分）；若要更精致，可把预渲染贴图换成 `tiles/*.png` 像素 Sprite、加帧动画 / 专业音轨——接口已留好（地形走预渲染 `Surface`、实体走 `_draw_*` 矢量，替换其一即可），仍须保持「只读 Game、零随机、确定性不变」。
