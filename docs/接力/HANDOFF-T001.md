...
1. 读 `CLAUDE.md` → （T-019 / T-020 已建，见 `docs/工单/T-019-灯开关独立实体.md`、`docs/工单/T-020-光照影响蜘蛛感应半径.md` 与对应 ADR-015 / ADR-016）。下一步候选（M19 完成后剩余唯一一条「光照影响蜘蛛感应半径」已落地为 M20）：
  ...
  - **（M21 已完成：可键盘操作玩法；M22 已完成：Pygame GUI 渲染层；M23 已完成：Spider-Man 主题化 GUI 渲染；M24 已完成：主题美术再升级 · 动画+美术+音效）**：M6–M24 的「视野 / 潜行 / 听觉 / 光照 / 蜘蛛感应 + 可键盘操作 + 窗口渲染 + 蜘蛛侠主题动画美术 + 音效」体系已闭环，游戏从「只能看 ASCII 终端 demo」变成「能在真实窗口里手动玩、角色会呼吸/浮动、攻击有蛛网行进与火花、暗夜有氛围光、且听得见脚步与摆荡」。后续新里程碑仍需先登记工单 + ADR + 不变量后再开工。候选方向（按优先级）：
    - **交互式射灯 / 碎灯 / 开关（M14/M16/M19 的玩家可控版）**：`_handle_key` 当前未覆盖「选定某个房间灯 / 墙边开关」的目标选择，仍由 demo 自动战术承担；可加「瞄准模式」（如先按某键进入瞄准、再用方向选目标格）让玩家手动射灭 / 射碎灯。
    - **Boss 战与胜利条件闭环**：当前演示在 MAX_DEPTH 清场即「演示结束」，没有真正的 Boss / 终局剧情；可加一个最终层 Boss（如绿魔）与通关画面，把无限下潜收口成一局完整的胜负。
    - **存档 / 读档（save/load）**：当前游戏状态不持久化，关掉终端即丢；可加序列化（注意 seed + 输入序列已能完整复现，存档本质就是存这两样）。
    - **更多蜘蛛侠招式（AoE 蛛网束缚 / 摆荡位移）**：把 M7 摆荡突袭扩展为可手动指定落点的位移技、或范围蛛网束缚多怪。
    - **主题美术换 tiles/*.png 序列帧 Sprite（M24 续，可选）**：M24 预渲染已按帧组织，若要更精致可把程序化贴图换成 `tiles/*.png` 序列帧（接口已留好），仍须保持「只读 Game、零随机、确定性不变」。
    - **网页化（Pygbag 把 Pygame 打包成 wasm，零改核心）**。
