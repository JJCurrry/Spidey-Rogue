...
## 当前状态
- 整体进度：M1 已完成（治理脚手架 + 格子移动）；M2 已完成（战斗系统）；M3 已完成（怪物 AI）；M4 已完成（道具与背包）；M5 已完成（程序化关卡）；**M6 已完成（视野 / 渲染层）**。
...
- 假设 I（M6 起）：`grid` 里只有 `#` / `.` / `@`（外加渲染层的 `M` / `!` / `>` / `?` / 空白，均不写回 grid）；
