...
1. 读 `CLAUDE.md` → （T-019 / T-020 已建，见 `docs/工单/T-019-灯开关独立实体.md`、`docs/工单/T-020-光照影响蜘蛛感应半径.md` 与对应 ADR-015 / ADR-016）。下一步候选（M19 完成后剩余唯一一条「光照影响蜘蛛感应半径」已落地为 M20）：
   ...
   - **（M21 已完成：可键盘操作玩法；M22 已完成：Pygame GUI 渲染层）**：M6–M22 的「视野 / 潜行 / 听觉 / 光照 / 蜘蛛感应 + 可键盘操作 + 窗口渲染」体系已闭环，游戏从「只能看 ASCII 终端 demo」变成「能在真实窗口里手动玩」。后续新里程碑仍需先登记工单 + ADR + 不变量后再开工。候选方向（按优先级）：
