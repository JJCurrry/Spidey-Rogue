...
## 当前状态
- 整体进度：M1 已完成（治理脚手架 + 格子移动）；M2 已完成（战斗系统）；M3 已完成（怪物 AI）；M4 已完成（道具与背包）；M5 已完成（程序化关卡）；M6 已完成（视野 / 渲染层）；M7 已完成（怪物视野与潜行）；M8 已完成（噪音与听觉）；**M9 已完成（主动制造响动 / 皇后区垃圾桶盖）**；**M10 已完成（ANSI 颜色高亮）**；**M11 已完成（光照衰减 / 明暗梯度）**；**M12 已完成（随身手电 / 动态光源）**。
...
1. 读 `CLAUDE.md` → 拉 `docs/工单/T-013*`（光照影响玩家自身视野：让暗处也压短玩家自己的视野半径；
   需让 `light` 关联 `fov` 且不引入随机，与 M6「视野是渲染层」边界协调——M12 只动了怪物感知侧，
   玩家视野仍按原逻辑）或新建。
